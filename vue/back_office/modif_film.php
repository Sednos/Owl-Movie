...
foreach($select_films as $select_film) {
    ...
    <div class="form-group col-xs-10">
     <form action="?module=back_office&action=modif_film&id=<?php echo $select_film['idfilm'];?>" method="post" enctype="multipart/form-data">
     Titre : <input class="form-control" type="text" name="titre" value="<?php echo $select_film['titre'];?>">
    Résumé : <textarea class="form-control" name='resume'><?php echo $select_film['resume']; ?></textarea>
    Acteurs : <input class="form-control" type="text" name="acteur" value="<?php echo $select_film['acteur'];?>">
    Réalisateur(s) : <input class="form-control" type="text" name="realisateur" value="<?php echo $select_film['realisateur'];?>"><br/>
    <input type="hidden" name="MAX_FILE_SIZE" value="4194304" />
    <label for="file">Affiche</label><input type="file" name="ch_file"/>
    <img src="<?php echo $select_film['affiche']; ?>" alt="Affiche">
    </div>
    <?php
    $date_fin = explode('-', $select_film['date_fin']);
    ?>
